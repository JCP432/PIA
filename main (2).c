...
char** split_str(char* str, const char a_delim) {
	...
	delim[0] = a_delim;
	delim[1] = '\0';
	//cuenta cuantos elementos seran extraidos
	while(*str_tmp) {
		if(*str_tmp == a_delim) {
			count++;
			last_delim = str_tmp;
	...
	//agregar espacio para el token final
...
void saveData_at_file(const char* calificaciones_pia_pe, alumno* alumnos, calificacionP* calificaciones, grupo g, const size_t tamA, const size_t tamC) {
	FILE* stream = fopen(calificaciones_pia_pe,"w");
	if(!stream) {
	...
	size_t i = 0, j = 0;
	size_t len = 0;
	fprintf(stream,"matricula,parcial1,parcial2,parcial3,parcial4,parcial5,parcial6,parcial7,Prom Calif Parciales,parciales reprobados\n");
	for(i = 0; i < tamA; ++i) {
...
int main() {
	...
	saveData_at_file("calif_finales.csv",alumnos,calificacionesP,g,TAM,7);
